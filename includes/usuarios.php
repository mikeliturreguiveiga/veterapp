...
if (isset($_POST['exportar_PDF'])) {
    ...
    $pdf->Cell(30, 10, iconv('UTF-8', 'windows-1252', 'Teléfono'),1);
    $pdf->Cell(70, 10, iconv('UTF-8', 'windows-1252', 'Correo electrónico'),1);
    $pdf->Cell(90, 10, iconv('UTF-8', 'windows-1252','Dirección'),1);
    $pdf->Ln();

    $pdf->SetFont("Arial", "", 10);

    $consulta_completa = "SELECT * FROM usuarios";
    ...
    while ($fila = $resultado_consulta_completa->fetch_assoc()) {
        ...
        $pdf->Cell(50, 10, iconv('UTF-8', 'windows-1252', $fila['apellidos']),1); 
        $pdf->Cell(30, 10, ($fila['telefono']),1); 
        $pdf->Cell(70, 10, ($fila['correo_electronico']),1); 
        $pdf->Cell(90, 10, iconv('UTF-8','windows-1252', $fila['direccion']),1);  
        $pdf->Ln();   
    }
    $pdf->Output();
    exit;
